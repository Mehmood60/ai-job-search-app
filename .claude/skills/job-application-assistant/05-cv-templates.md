...
> **Active template override: `single-column-red`**
>
> A custom template is active. Where this block conflicts with the stock guidance below, this block wins. Structural advice below (tailoring, relevance-weighted cutting rules) still applies.
>
> - **Template skeleton:** `templates/cv/single-column-red/template.tex` — use this as the structural reference instead of the stock moderncv template
> - **Manifest:** `templates/cv/single-column-red/TEMPLATE.md` — read this for style rules and known pitfalls before drafting
> - **Compile with:** `lualatex` (NOT the moderncv command in the stock guidance below)
> - **Fonts:** Latin Modern (default, ships with TeX — nothing to install)
> - **Page limit:** NONE — this is a multi-page design; do NOT force it to 2 pages
> - **Required asset:** a `picture.jpeg` headshot must be copied into the output directory (`cv/`) before compiling — the header clips it to a circle; the compile fails without it
> - **Output file:** unchanged (`cv/main_<company>.tex`); copy `picture.jpeg` into `cv/` alongside it
<!-- END ACTIVE-TEMPLATE -->
...
<!-- These are populated based on Mehmood's background. Tailor the specifics per posting. -->
**For Full Stack Engineer roles:**
> Full Stack Engineer with 5+ years building and shipping production web applications end-to-end. Specializes in React, Next.js, and TypeScript on the frontend and Node.js and PHP on the backend, backed by PostgreSQL/MySQL, Docker, and CI/CD. Has owned systems from architecture through live deployment — including a production billing platform and a self-hosted eBay operations platform deployed live with Docker and automatic HTTPS. Former startup CTO who ships fast without sacrificing quality (85%+ test coverage).

**For Frontend Engineer roles:**
> Frontend-focused engineer with 5+ years building responsive, accessible web UIs in React, Next.js, TypeScript, and Angular with TailwindCSS. Delivers pixel-perfect interfaces from Figma, optimizes performance (25% bundle-size reductions via code splitting and lazy loading), and enforces WCAG accessibility — with full-stack awareness that makes frontend work integrate cleanly with Node.js/PHP APIs.

**For Backend / PHP / Node.js Engineer roles:**
> Backend engineer with 5+ years designing REST APIs and data models in Node.js/Express and PHP (CakePHP), serving platforms at 1M+ requests/month. Builds robust financial and data-processing workflows with structured validation, idempotency, and error handling; optimizes SQL (40% response-time reductions, P95 800ms→200ms); and deploys with Docker and CI/CD. One year of production Python for backend APIs and ML-service integration.

**For HealthTech / B2B SaaS roles (sector emphasis):**
> Full Stack Engineer with 5+ years across B2B SaaS and HealthTech — from telemedicine mobile apps (hello-doctor.de, hello-sister.de) to a production veterinary billing and insurance platform. Combines end-to-end product ownership with a passion for improving outcomes through technology, and the startup instinct to take a product from zero to hundreds of B2B users.
