...
- **Always write in English**, regardless of the posting's language (per CLAUDE.md "Language rule"). Do NOT match the posting language unless the user explicitly asks for a specific application.
...
**Use the compile engine and page limit declared by the ACTIVE-TEMPLATE block** at the top of `05-cv-templates.md` / `06-cover-letter-templates.md` (you read these in Step 2). A custom template overrides the stock defaults below — e.g. the active CV template may compile with a different engine and have **no 2-page cap**, and the active cover-letter template may require `pdflatex` (Helvetica) rather than `xelatex`. Copy any required assets the template names (e.g. `picture.jpeg`) into the output directory before compiling. Only fall back to the stock commands below when no custom template is active.
...
- [ ] Page count matches the active template's declared page limit. The stock moderncv template is a hard **2 pages**; a custom template may declare a different limit or **no cap** (e.g. `single-column-red` is intentionally multi-page) — in that case do NOT trim content to hit 2 pages.
...
- [ ] Section headings are not isolated at the bottom/top of a page with only 1-2 lines below
- [ ] No awkward whitespace gaps (a final page under ~half full on an uncapped template is acceptable; do not pad or cut)

**Cover letter (`cover_letters/cover_<company>_<role>.pdf`):**
- [ ] Exactly 1 page
- [ ] Signature block visible, not cut off or pushed to a second page
- [ ] Bullet/body font matches the active template's font (stock cover.cls = Raleway-Medium; `redblood-cover` = Helvetica via pdflatex)
...
### 5f. Export to the jobs folder (MANDATORY)

Every `/apply` run must archive its inputs and outputs to the user's external jobs directory:

**Base path:** `C:\Users\MehmoodulHassan\OneDrive - VetControl GmbH\Desktop\CVs\jobs`

1. Create a company subfolder: `<base>\<CompanyName>` (use the parsed company name; if it already exists from a prior run, reuse it and overwrite the files inside).
2. Save the **original job description** as `job_description.txt` in that folder — the full posting text (or, if a URL was fetched, the extracted posting plus the source URL and capture date). This is the raw JD, not the fit evaluation.
3. Copy the final compiled PDFs into that folder with clear names:
   - `Mehmood Ul Hassan - CV - <Company>.pdf`
   - `Mehmood Ul Hassan - Cover Letter - <Company>.pdf`
4. Also copy the `.tex` sources (`main_<company>.tex`, `cover_<company>_<role>.tex`) into the folder for future editing.
5. Confirm the folder contents to the user in Step 6.

Use PowerShell `Copy-Item ... -Force` (the base path contains spaces and must be quoted). The repo copies under `cv/` and `cover_letters/` remain the working copies; the jobs folder is the delivered archive.
...
### Files Created
List the files written (repo working copies):
- `cv/main_<company>.tex` + `.pdf`
- `cover_letters/cover_<company>_<role>.tex` + `.pdf`

And the exported archive (Step 5f):
- `C:\Users\MehmoodulHassan\OneDrive - VetControl GmbH\Desktop\CVs\jobs\<Company>\` — `job_description.txt`, both PDFs, both `.tex` sources
