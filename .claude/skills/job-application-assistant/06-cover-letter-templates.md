...
> **Active template override: `redblood-cover`**
>
> A custom template is active. Where this block conflicts with the stock guidance below, this block wins. Structural advice below (writing style, 1-page limit, forward-looking framing) still applies.
>
> - **Template skeleton:** `templates/cover_letters/redblood-cover/template.tex` — use this instead of the stock cover.cls structure
> - **Manifest:** `templates/cover_letters/redblood-cover/TEMPLATE.md` — read for style rules and known pitfalls before drafting
> - **Compile with:** `pdflatex` (NOT xelatex/cover.cls, and NOT lualatex — the Helvetica `\fontfamily{phv}` only renders under pdflatex; lualatex/xelatex fall back to serif)
> - **Fonts:** Helvetica (`phv`) body + fontawesome5 icons (all ship with TeX — nothing to install; the stock `cover.cls`/OpenFonts pitfalls below do NOT apply)
> - **Page limit:** exactly 1 page
> - **Body pattern:** put the whole letter inside `\content{ ... }` followed immediately by `\signature` (the `\signature` token is captured as `\content`'s 2nd argument — do not remove it). Match sub-labels use `\textcolor{redBlood}{\textbf{...}}`
> - **Output file:** unchanged (`cover_letters/cover_<company>_<role>.tex`)
<!-- END ACTIVE-TEMPLATE -->
